...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // PHẦN 3: Chỉnh sửa ảnh đại diện (HinhAnh)
    setupAvatarEditor(currentUser);
});
...
async function loadProfileData(user) {
    ...
    // PHẦN 3: Hiển thị ảnh đại diện từ cột HinhAnh (hoặc placeholder theo tên)
    applyAvatarToUI(user);
    const avatarUrlInput = document.getElementById('profileAvatarUrl');
    if (avatarUrlInput) avatarUrlInput.value = user.HinhAnh || '';
    ...
    if (profileForm) {
        profileForm.addEventListener('submit', async (e) => {
            ...
            const updatePayload = {
                ...
                SDT: user.SDT,
                MaLoaiUser: user.MaLoaiUser,
                HinhAnh: user.HinhAnh || '' // PHẦN 3: giữ ảnh đại diện khi đổi mật khẩu
            };
...
/**
 * PHẦN 3: Áp dụng URL ảnh đại diện lên thẻ <img>
 */
function applyAvatarToUI(user) {
    ...
    if (user.HinhAnh && user.HinhAnh.trim() !== '') {
        avatarImg.src = user.HinhAnh;
        avatarImg.onerror = () => {
            const firstLetter = (user.Ten || 'U').trim().charAt(0).toUpperCase();
...
/**
 * PHẦN 3: Mở form sửa avatar, lưu qua API PATCH /api/customers/:id/avatar
 */
function setupAvatarEditor(user) {
    const trigger = document.getElementById('avatarEditTrigger');
    const panel = document.getElementById('avatarEditPanel');
    ...
    trigger.addEventListener('click', () => {
        ...
        if (urlInput) urlInput.value = user.HinhAnh || '';
    });
    ...
    if (saveBtn) {
        saveBtn.addEventListener('click', async () => {
            ...
            try {
                const response = await fetch(`/api/customers/${user.MaKH}/avatar`, {
                    method: 'PATCH',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ HinhAnh: newUrl })
                });
                if (!response.ok) {
                ...
                console.error(error);
                alert(error.message || 'Có lỗi khi lưu ảnh đại diện');
            } finally {
