...
BEGIN
    ...
    BEGIN TRY
        ...
        VALUES (@Ten, @TenDangNhap, @MatKhau, @Email, @SDT, @MaLoaiUser, @HinhAnh)
        COMMIT TRANSACTION
    END TRY
    BEGIN CATCH
        IF @@TRANCOUNT > 0 ROLLBACK TRANSACTION;
...
CREATE PROCEDURE sp_UpdateLoaiSP
    @id INT,
    @TenLoaiSP NVARCHAR(100)
AS
BEGIN
    ...
    BEGIN TRY
        BEGIN TRANSACTION;
        UPDATE LoaiSanPham SET TenLoaiSP = @TenLoaiSP WHERE MaLoaiSP = @id;
        COMMIT TRANSACTION;
    END TRY
    BEGIN CATCH
        IF @@TRANCOUNT > 0 ROLLBACK TRANSACTION;
...
-- Xóa một Loại SP
CREATE PROCEDURE sp_DeleteLoaiSP
    @id INT
AS
BEGIN
    SET NOCOUNT ON;
    BEGIN TRY
        BEGIN TRANSACTION;
        DELETE FROM LoaiSanPham WHERE MaLoaiSP = @id;
        COMMIT TRANSACTION;
    END TRY
    BEGIN CATCH
        IF @@TRANCOUNT > 0 ROLLBACK TRANSACTION;
...
BEGIN
    ...
    BEGIN TRY
        ...
        VALUES (@TenSP, @Gia, @HinhAnh, @MoTa, @MaLoaiSP);
        COMMIT TRANSACTION;
    END TRY
    BEGIN CATCH
        IF @@TRANCOUNT > 0 ROLLBACK TRANSACTION;
...
BEGIN
    ...
    BEGIN TRY
        BEGIN TRANSACTION;
        UPDATE SanPham SET TenSP = @TenSP, Gia = @Gia, HinhAnh = @HinhAnh, MoTa = @MoTa, MaLoaiSP = @MaLoaiSP
        WHERE MaSP = @id;
        COMMIT TRANSACTION;
    END TRY
    BEGIN CATCH
...
BEGIN
    ...
    FROM KhachHang k
    JOIN LoaiUser l ON k.MaLoaiUser = l.MaLoaiUser
    WHERE (k.TenDangNhap = @username OR k.Email = @username OR k.SDT = @username) 
...
BEGIN
    ...
    BEGIN TRY
        ...
        VALUES (@MaHoaDon, @MaPhim, @MaSuat, @MaGhe, @GiaVe)
        COMMIT TRANSACTION
    END TRY
    BEGIN CATCH
        IF @@TRANCOUNT > 0 ROLLBACK TRANSACTION;
...
BEGIN
    ...
    BEGIN TRY
        ...
        OUTPUT INSERTED.MaHoaDon, INSERTED.MaQR
...
BEGIN
    ...
    BEGIN TRY
        ...
        VALUES (@TenBanner, @LinkBanner, @MaPhim);
        COMMIT TRANSACTION;
    END TRY
    BEGIN CATCH
        -- Trả lại trạng thái nếu có lỗi xảy ra trong quá trình thêm mới
        IF @@TRANCOUNT > 0 ROLLBACK TRANSACTION;
        DECLARE @err NVARCHAR(MAX) = ERROR_MESSAGE();
        RAISERROR(@err, 16, 1);
    END CATCH
...
CREATE PROCEDURE sp_UpdateBanner
    @id INT,
    @TenBanner NVARCHAR(50),
    @LinkBanner NVARCHAR(MAX),
    @MaPhim INT
...
BEGIN
    ...
    BEGIN TRY
        ...
        UPDATE Banner
        SET TenBanner = @TenBanner, LinkBanner = @LinkBanner, MaPhim = @MaPhim
        WHERE MaBanner = @id;
        COMMIT TRANSACTION;
    END TRY
    BEGIN CATCH
        -- Hủy bỏ giao dịch nếu xuất hiện lỗi
        IF @@TRANCOUNT > 0 ROLLBACK TRANSACTION;
        DECLARE @err NVARCHAR(MAX) = ERROR_MESSAGE();
        RAISERROR(@err, 16, 1);
    END CATCH
...
CREATE PROCEDURE sp_DeleteBanner
    @id INT
AS
BEGIN
    SET NOCOUNT ON;
    ...
    END TRY
    BEGIN CATCH
        -- Phục hồi lại dữ liệu nếu có sự cố xảy ra
        IF @@TRANCOUNT > 0 ROLLBACK TRANSACTION;
        DECLARE @err NVARCHAR(MAX) = ERROR_MESSAGE();
        RAISERROR(@err, 16, 1);
    END CATCH
...
CREATE PROCEDURE sp_GetCommentsByMovie
    @MaPhim INT
AS
BEGIN
    SET NOCOUNT ON;
    ...
    WHERE bl.MaPhim = @MaPhim AND bl.TrangThai = N'Hiện'
...
BEGIN
    ...
    BEGIN TRY
        ...
        VALUES (@MaPhim, @MaKH, @NoiDung, @SoSao);
        SELECT SCOPE_IDENTITY() AS MaBL;
        COMMIT TRANSACTION;
    END TRY
    BEGIN CATCH
        IF @@TRANCOUNT > 0 ROLLBACK TRANSACTION;
...
CREATE PROCEDURE sp_UpdateCustomerAvatar
    @id INT,
    @HinhAnh NVARCHAR(MAX)
AS
BEGIN
    ...
    BEGIN TRY
        BEGIN TRANSACTION;
        UPDATE KhachHang SET HinhAnh = @HinhAnh WHERE MaKH = @id;
        COMMIT TRANSACTION;
    END TRY
    BEGIN CATCH
        IF @@TRANCOUNT > 0 ROLLBACK TRANSACTION;
